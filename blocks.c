...
static int dwmblocks(void) {
    if (setlocale(LC_ALL, "") == NULL) {
        error(__func__, "Error setting locale. Check your locale configuration.\n");
        exit(EXIT_FAILURE);
    }
    {
    ...
    if ((display = XOpenDisplay(NULL)) == NULL) {
        error(__func__, "Error opening X display\n");
        exit(EXIT_FAILURE);
    }
    ...
    while (true) {
        ...
        if (ready < 0) {
            if (errno == EINTR) {
                ...
                error(__func__, "Error polling: %s\n", strerror(errno));
                exit(EXIT_FAILURE);
            }
        }
        if (ready > 0) {
            ...
            for (int i = 0; i < LENGTH(blocks); i += 1) {
                ...
                if (pipes[i].revents & POLLHUP) {
                    ...
                    error(__func__, "Error polling: Invalid fd.\n");
                    pipes[i].fd = -1;
                } else if (pipes[i].revents & POLLERR) {
                    error(__func__, "Error polling: Error condition.\n");
                    pipes[i].fd = -1;
                }
                if (block->function)
